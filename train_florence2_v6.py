#!/usr/bin/env python3
"""
train_florence2_v6.py
=====================
Florence-2-large fine-tuning v6 — LoRA on BOTH vision encoder + language decoder.

Changes from v5:
 - LoRA now targets vision encoder (DaViT) modules: qkv, proj
 - Combined with existing language decoder targets: q_proj, v_proj, k_proj, o_proj, out_proj, fc1, fc2
 - This allows the model to learn better visual features for food pantry items
 - Per Prof. Zizhan's recommendation
...
def main():
    parser = argparse.ArgumentParser(description="Florence-2-large fine-tuning v6 (vision+language LoRA)")
    ...
    parser.add_argument("--lr", type=float, default=3e-5,
                        help="Peak learning rate (slightly lower for dual LoRA)")
    parser.add_argument("--weight-decay", type=float, default=0.01)
    parser.add_argument("--warmup-ratio", type=float, default=0.06)
    parser.add_argument("--max-length", type=int, default=512)
    parser.add_argument("--lora-r", type=int, default=32,
                        help="LoRA rank (lower than v5 since more modules are targeted)")
    ...
    lora_config = LoraConfig(
        ...
        target_modules=[
            ...
            "qkv",       # spatial & channel attention Q/K/V (combined)
            "proj",      # attention output projection
            # ── Language decoder ──
            "q_proj",    # decoder self-attention
            "v_proj",
            "k_proj",
            "o_proj",
            "out_proj",
            # ── Shared FFN (both vision & decoder) ──
            "fc1",
            "fc2",
        ],
        bias="none",
    ...
    print(f"  LoRA targets: vision encoder (qkv, proj) + language decoder (q/k/v/o_proj, out_proj) + FFN (fc1, fc2)")
    ...
    print(f"\n{'='*60}")
    print("STARTING TRAINING (v6 — Vision + Language LoRA)")
    print(f"{'='*60}\n")
