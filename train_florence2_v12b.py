#!/usr/bin/env python3
"""
train_florence2_v12b.py
=======================
Florence-2-large fine-tuning v12b — Conservative multi-label improvement.

V12 was too aggressive (3x multi-item boost + weighted CE → hallucinations, F1 73.6%).
V12b is a conservative retry from v11 checkpoint:

Strategy:
  - From v11 checkpoint (best single model: 76.0% Micro F1)
  - 1.5x multi-item boost (NOT 3x like v12)
  - Standard CE loss, NO weighted per-sample loss (uniform, like v11)
  - Only 5 worst recall classes boosted (NOT 7 like v12)
  - Boost intensity: 30% (NOT 60% like v12)
  - LR 1.5e-5 (between v11's 1e-5 and v12's 2e-5)
  - 12 epochs, patience 5
  - Case normalization in targets

Goal: v11 + slightly more multi-label exposure without over-correcting.
...
class Florence2PantryDatasetV7(Dataset):
    ...
    # v12b: Only boost the 5 worst recall classes from v11 results
    # (same 5 as v11, but now using v11's actual recall numbers)
    CONFUSION_BOOST_CLASSES = {
        "Vegetables - Fresh",           # 33.3% recall in v11 (3 support)
        "Ready Meals",                  # 35.3% recall in v11 (17 support)
        "Granola Products",             # 50.0% recall in v11 (8 support)
        "Dairy and Dairy Alternatives", # 55.6% recall in v11 (9 support)
        "Meat and Poultry - Canned",    # 57.1% recall in v11 (7 support)
    }
    ...
    def _oversample_v7(self, samples, min_samples_per_class, confusion_boost):
        ...
        # 2. Multi-item boost: 1.5x (add full set once + half set)
        # v11 did 1x, v12 did 3x (too aggressive). 1.5x is the sweet spot.
        if multi_item_samples:
            result.extend(multi_item_samples)  # +1x
            half = multi_item_samples[:len(multi_item_samples) // 2]
            result.extend(half)  # +0.5x
            print(f"    Multi-item boost: +{len(multi_item_samples) + len(half)} samples (~1.5x)")

        # 3. Low-recall class boost (30% intensity — conservative)
        if confusion_boost:
            for cls in self.CONFUSION_BOOST_CLASSES:
                if cls in class_samples:
                    boost_count = max(5, int(len(class_samples[cls]) * 0.3))
...
def main():
    parser = argparse.ArgumentParser(description="Florence-2-large fine-tuning v12b (conservative multi-label from v11)")
    parser.add_argument("--data-dir", type=str, default=".")
    parser.add_argument("--jsonl-dir", type=str, default="./florence2_data")
    parser.add_argument("--output-dir", type=str, default="./checkpoints_v12b")
    ...
    parser.add_argument("--lr", type=float, default=1.5e-5,
                        help="Between v11 (1e-5) and v12 (2e-5)")
    ...
    parser.add_argument("--focal-gamma", type=float, default=0.0,
                        help="0 = standard CE loss (no focal). Keep it simple like v11.")
    parser.add_argument("--label-smoothing", type=float, default=0.03,
                        help="Same light smoothing as v11")
    ...
    print(f"\nTraining config (v12b — Conservative multi-label from v11):")
    ...
    print(f"\n{'='*60}")
    print("STARTING TRAINING (v12b — Conservative Multi-label from v11)")
    print(f"{'='*60}\n")
    ...
    print(f"\n{'='*60}")
    print("TRAINING COMPLETE (v12b — Conservative Multi-label from v11)")
    print(f"{'='*60}")
