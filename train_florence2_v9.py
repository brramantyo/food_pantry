#!/usr/bin/env python3
"""
train_florence2_v9.py
=======================
Florence-2-large fine-tuning v9 — Grocery Store Dataset pre-training + food pantry fine-tuning.

Changes from v7:
 - Reduced focal gamma (2.0 → 1.0) to prevent over-correction
 - Removed high-support classes from confusion boost (Carbohydrate Meal,
   Bread, Desserts already have enough samples — boosting them caused
   them to dominate predictions in v7)
 - Only boost truly low-recall + low-support classes
 - Reduced oversampling cap (30 → 25) to avoid flooding with duplicates
 - Kept all v7 improvements that worked: multi-item boost, LoRA r=48,
   perspective augmentation, label smoothing

Target: Keep v7 recall gains while restoring precision to >82%.
...
class Florence2PantryDatasetV7(Dataset):
    ...
    def _oversample_v7(self, samples, min_samples_per_class, confusion_boost):
        ...
        # 3. Confusion-prone class boost (extra 30%, gentler than v7's 50%)
        if confusion_boost:
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="./checkpoints_v9")
    ...
    parser.add_argument("--train-jsonl", type=str, default=None,
                        help="Override training JSONL path (e.g. merged grocery+pantry data)")
    ...
    print(f"\n{'='*60}")
    print("STARTING TRAINING (v9 — Balanced Recall Fix)")
    print(f"{'='*60}\n")
