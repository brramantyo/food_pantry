#!/usr/bin/env python3
"""
train_florence2_v8.py
=====================
Florence-2-large fine-tuning v8 — Multi-item detection focus.

Root cause analysis from v7.1:
 - Single-item accuracy: 79.6% (decent)
 - Multi-item accuracy: 0.0% (complete failure)
 - Model predicts avg 0.4 items for images with avg 2.6 items
 - 62.8% of training is single-item, model defaults to single output

Changes from v7.1:
 - Heavy multi-item oversampling (3x) to balance single vs multi
 - Remove 0-item samples (noise, 19 samples)
 - Multi-item curriculum: extra weight on 3+ item samples (4x)
 - Increased max_length 512 → 768 for longer multi-item JSON
 - Keep all v7.1 improvements (focal loss gamma=1.0, LoRA r=48, etc.)

Target: Multi-item accuracy 0% → 30%+, overall F1 73.7% → 80%+.
...
class Florence2PantryDatasetV7(Dataset):
    ...
    # Only boost classes that are BOTH low-recall AND low-support
    # Removed: Desserts (30 samples), Bread (14, high recall), Carbohydrate Meal (23, dominated v7)
    CONFUSION_BOOST_CLASSES = {
        "Granola Products",             # 8 support, was 25% recall in v6
        "Dairy and Dairy Alternatives", # 9 support, was 33% recall in v6
        "Vegetables - Fresh",           # 3 support, was 33% recall in v6
        "Meat and Poultry - Canned",    # 7 support, was 43% recall in v6
        "Ready Meals",                  # 17 support, was 35% recall in v6
        "Condiments and Sauces",        # 15 support, was 47% recall in v6
    }
    ...
    def __init__(self, jsonl_path, data_dir, processor, max_length=1024,
        ...
        with open(jsonl_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if line:
                    ...
                    # v8: Remove 0-item samples (noise)
                    try:
        ...
        print(f"  Loaded {len(raw_samples)} raw samples from {jsonl_path} (skipped {skipped_empty} empty)")
    ...
    def _oversample_v7(self, samples, min_samples_per_class, confusion_boost):
        class_samples = {}
        multi_item_samples = []
        multi_3plus_samples = []  # v8: track 3+ item samples separately
        ...
        print(f"  Multi-item samples: {len(multi_item_samples)} (2+), {len(multi_3plus_samples)} (3+)")
        ...
        # 2. v8: HEAVY multi-item boost (3x for 2-item, 4x for 3+ items)
        if multi_item_samples:
            # Add 2-item samples 3 extra times
            result.extend(multi_item_samples * 3)
            print(f"    Multi-item boost (2+): +{len(multi_item_samples) * 3} samples (3x)")
        if multi_3plus_samples:
            # Add 3+ item samples 1 more extra time (total 4x including above)
            result.extend(multi_3plus_samples)
            print(f"    Multi-item boost (3+): +{len(multi_3plus_samples)} samples (extra 1x)")

        # 3. Confusion-prone class boost (extra 30%)
        if confusion_boost:
            for cls in self.CONFUSION_BOOST_CLASSES:
                if cls in class_samples:
                    boost_count = max(3, len(class_samples[cls]) // 3)
...
def main():
    ...
    parser.add_argument("--min-samples-per-class", type=int, default=25)
    ...
    print(f"\n{'='*60}")
    print("STARTING TRAINING (v8 — Multi-Item Focus)")
    print(f"{'='*60}\n")
